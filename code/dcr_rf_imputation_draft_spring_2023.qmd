...
import plotly.io as pio
    #* user defined
from fun import simulate, ampute, transformation, impute, create_iter_table
# Seed
...
![X](../assets/pop_x_dist.png){#fig-population-dist-x}

![Z](../assets/pop_z_dist.png){#fig-population-dist-z}

![Y](../assets/pop_y_dist.png){#fig-population-dist-y}

![X](../assets/amputed_x_dist.png){#fig-amputed-dist-x}

![Z](../assets/amputed_z_dist.png){#fig-amputed-dist-z}

![Y](../assets/amputed_y_dist.png){#fig-amputed-dist-y}
...
engine <- dbConnect(duckdb(), dbdir = "../data/sim_original.db")
engine2 <- dbConnect(duckdb(), dbdir = "../data/sim_amputed.db")
...
Overall, we see that the procedures yield small differences between the actual and estimated values. For Y, `AMELIA II` has a mean discrepancy of ```r sprintf('%.3f', mean(amelia_diff$mean_y)) ```, the Linear `MICE` procedure has a mean discrepancy of ```r sprintf('%.3f', mean(lmice_diff$mean_y)) ```, the interpolation procedure has a mean discrepancy of ```r sprintf('%.3f', mean(interpolate_diff$mean_y)) ```, and `RF-MICE` has a mean discrepancy of ```r sprintf('%.3f', mean(rfmice_diff$mean_y)) ```. For X, we see that the `RF-MICE` has a mean discrepancy of ```r sprintf('%.3f', mean(rfmice_diff$mean_x)) ```, interpolation has a mean discrepancy of ```r sprintf('%.3f', mean(interpolate_diff$mean_x)) ```, Linear `MICE` has a mean discrepancy of ```r sprintf('%.3f', mean(lmice_diff$mean_x)) ```, and AMELIA has a mean discrepancy of ```r sprintf('%.3f', mean(amelia_diff$mean_x)) ```. For Z, we also see that `RF-MICE` has a mean discrepancy of ```r sprintf('%.3f', mean(rfmice_diff$mean_z)) ```, interpolation has a mean discrepancy of ```r sprintf('%.3f', mean(interpolate_diff$mean_z)) ```, Linear `MICE` has a mean discrepancy of ```r sprintf('%.3f', mean(lmice_diff$mean_z)) ``` and `AMELIA II` has a mean discrepancy of ```r sprintf('%.3f', mean(amelia_diff$mean_z)) ```. 
