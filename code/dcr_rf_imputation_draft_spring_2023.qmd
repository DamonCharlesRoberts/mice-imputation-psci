...
lwd_diff = discrepancy(imputed = amputed, original = original)        
        #** Mean
...
ggplot() +
    geom_boxplot(aes(x = "LWD", y = mean_x), data = lwd_diff) +
...
ggplot() +
    geom_boxplot(aes(x = "LWD", y = mean_z), data = lwd_diff) +
...
ggplot() +
    geom_boxplot(aes(x = 'LWD', y = mean_y), data = lwd_diff) +
...
for(i in range(datasets)){
    ...
    amputed_list <- list(
        N = nrow(amputed[[i]]),
        x = complete.cases(amputed[[i]]$X),
        z = complete.cases(amputed[[i]]$Z),
        y = complete.cases(amputed[[i]]$Y)
    )
    stanfit <- sampling(compiled, amputed_list)
    lwd[[i]] <- as.data.frame(stanfit)
}
