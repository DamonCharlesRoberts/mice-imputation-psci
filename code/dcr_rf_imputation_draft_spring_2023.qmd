...
To examine this claim, I take the amputed and imputed datasets (10) and use Rubin's rule to pool across the regression models performed on each amputed and imputed sample. In total, I fit $`m` \times `s` $ -- $10 \times 1000$ regressions where I pool across my average posterior draws from `m` to produce a single set of statistics for each `s`. I calculate the discrepancy of the pooled average posterior draws for `X` and `Z` -- the variables directly contributing to the DGP of `Y` -- from their parameters. I then present boxplots that demonstrates the differences in levels of bias between `LWD`, `Amelia`, Linear `MICE`, and `RF-MICE` across my samples. The specification for the regression is depicted in @eq-regression. 
...
It is important to remind the reader that all tools have their limitations and that their value are quite dependent on the context for which they are to be applied. It is useful, however, to include tools that vary in the assumptions they make [@neumayer_plumper_2017]. The capabilities of `RF-MICE` are no different. While `RF-MICE` is quite flexible for addressing a number of `MAR` processes, it is important to note that it is not and should not be seen as a default or the sole tool for one to use when dealing with missing data. With the number of tools and their implementations being so available to the applied researcher, one should not shy away from using multiple implementations of these tools to ensure that one's substantive conclusions are not dependent on one tool or implementation.
<!--
    END OF DOCUMENT
-->