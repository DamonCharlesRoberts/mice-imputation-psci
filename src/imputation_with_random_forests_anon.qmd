...
import polars as pl
    #* user defined
from PY.helper import simulate, ampute, create_iter_table
# Seed
...
box::use(
  ,DBI[
    dbConnect
    ,dbDisconnect
    #,dbGetQuery, dbWriteTable
  ]
  ,duckdb[
    duckdb
  ], #needs to be version 0.6.1 for both R and python
...
amputedList <- lapply(
    amputeList
    , function(x) {
        amputedDF <- miceRanger::amputeData(
            data=x
            ,perc=0.4
            ,cols=c("X","Z","Y")
        )
...
amputedDF <- data.table::rbindlist(amputedList)
```
...
sample_connection=db.connect("../data/sim_original.db")
...
# Open database connection
amputed_connection=db.connect("../data/sim_amputed.db")
...
if reproduce == "true":
    ...
    for i in range(datasets):
        ...
        create_iter_table(
            ...
            name_append = "amputed",
            engine = amputed_connection,
            i = i
        )
# Close connection
amputed_connection.close()
```
```{r}
#| label: load-simulated-samples

# Open database connection
sampleConnection <- dbConnect(
    duckdb(),
...
<!--
```{r}
#| label: load-amputed-data

# Open database connection
amputedConnection <- dbConnect(
    duckdb(),
    dbdir = "../data/sim_amputed.db",
    read_only = TRUE # read_only mode
)
# Access amputed data
amputed <- organize_data(
    type="amputed"
    ,con=amputedConnection
)
```
-->
```{r}
...
[^5]: It is important to note that these benchmarks are based on a computer 16 GB of RAM, on a AMD Ryzen 9 6900HX processor at 3.30GHz with a Radeon Graphics Card.
...
procedureName <- c("interpolate", "amelia", "lmice", "rfmice", "rfranger")
discrepancyList <- lapply(
    procedureName
    ,function(x) {
        discrepancy(
            sample_data=sample
            ,imputed_data=imputedList
            ,procedure=x
            ,model=FALSE
        )
    }
)
base::names(discrepancyList) <- procedureName
discrepancyDF <- data.table::rbindlist(discrepancyList, idcol = "procedure")
discrepancyDF <- discrepancyDF[
